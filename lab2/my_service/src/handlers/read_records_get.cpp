...
#include "../db/readmodel_queries.hpp"
...
ReadRecordsGet::ReadRecordsGet(const userver::components::ComponentConfig& config,
                               const userver::components::ComponentContext& context)
    : HttpHandlerJsonBase(config, context) {
  pg_ = context.FindComponent<userver::components::Postgres>("postgres-db").GetCluster();
}

userver::formats::json::Value ReadRecordsGet::HandleRequestJsonThrow(
  ...
  const auto res = pg_->Execute(userver::storages::postgres::ClusterHostType::kSlave,
                                myservice::db::kGetReadModelRecordById,
                                record_id);
  if (res.IsEmpty()) {
    throw userver::server::handlers::ResourceNotFound(
        userver::server::handlers::ExternalBody{"Record not found in read model"});
  }

  const auto row = res.Front();
  userver::formats::json::ValueBuilder out;
  out["id"] = row["record_id"].As<std::int64_t>();
